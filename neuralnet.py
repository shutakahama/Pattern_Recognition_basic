...
class MLP():
    ...
    def train(self, x, t, lr):     
        ...
        for layer in self.layers[-2::-1]:
            # dL/du = (dL(-1)/du*W)*f'(u)
            delta = np.dot(delta, W.T) * layer.activation.deriv(layer.u)
            layer.delta = delta
            W = layer.W
...
plt.legend()
plt.savefig("./result/neuralnet.png")
# plt.show()
